...
{
    internal struct LibraryFileContext
    {
        ...
        {
            return Helpers.TryMatchPath(_context, _prefixUrl, forDirectory: false, subpath: out PathString libraryPath) &&
                   Helpers.TryMatchLibrary(libraryPath, _pathProvider.GetLibraries(), forDirectory: false, subpath: out _subPath, library: out _library);
        }

        public bool LookupContentType()
        {
            _fileProvider = new PhysicalFileProvider(_library.Path);
        ...
        public void ApplyResponseHeaders(int statusCode)
        {
            ...
            _options.OnPrepareResponse(new LibraryFileResponseContext()
            {
                Context = _context,
                File = _fileInfo,
            });
        ...
        public async Task SendAsync()
        {
            ...
            if (sendFile != null && !string.IsNullOrEmpty(physicalPath))
            {
                ...
                _logger.LogTransportMethod("SendFile");
            ...
            try
            {
                ...
                {
                    _logger.LogTransportMethod("StreamCopy");
        ...
        internal async Task SendRangeAsync()
        {
            ...
            {
                _logger.LogTransportMethod("SendFile");
                _logger.LogSendingFileRange(_response.Headers[HeaderNames.ContentRange], physicalPath, _context.User?.Identity?.Name);
                // We don't need to directly cancel this, if the client disconnects it will fail silently.
            ...
            try
            {
                using (var readStream = _fileInfo.CreateReadStream())
                {
                    ...
                    _logger.LogTransportMethod("StreamCopy");
                    _logger.LogCopyingFileRange(_response.Headers[HeaderNames.ContentRange], SubPath, _context.User?.Identity?.Name);
